...
export const Sidebar = () => {
  return (
    <div className="fixed right-0 top-[50%] -translate-y-1/2 flex items-center justify-center p-12 z-50 font-sans">
      <aside className="relative h-[600px] transition-all duration-500 ease-in-out border-4 border-black rounded-l-3xl shadow-[2px_2px_0_0_rgba(0,0,0,1)] overflow-y-auto overflow-x-hidden bg-[#E9EBE0]">
        <ul className="p-4 space-y-3">
          {links.map((item, i) => {
            const isSelected = item.label === "Ngee Ann";
            return (
              <li
                key={i}
                className={`${
                  item.color
                } flex items-center justify-start border-2 ${
                  isSelected ? "border-black" : "border-black"
                } rounded-md p-2 shadow-[1px_1px_0_0_rgba(0,0,0,1)] transition-all duration-500 ease-in-out hover:bg-opacity-80 w-full h-12`}
              >
                <Link
                  href={item.href}
